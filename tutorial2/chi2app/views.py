...
def get_Analysis(df):
    ...
    # 커피숍 이름을 숫자로
    df['coNum'] = df['co_survey'].apply(lambda c:1 \
                                            if c =='스타벅스' else 2 \
                                            if c == '커피빈' else 3 \
                                            if c == '이디아' else 4)
    ...
    if pv > 0.05 :
        result = "p-value 값이 유의수준 <b>{:.3f} > 0.05</b> 이므로, "\
...
def view_Visualization(df):
    ...
    # 그룹화 하기
    gen_gruop = df['co_survey'].groupby(df['coNum']).count()
    
    # 그룹에 인덱스 이름 정의하기 (인덱스 번호를 한글 이름으로)
    gen_gruop.index = ['스타벅스','커피빈','이디아','탐앤탐스']
    
    # 막대그래프 그리기
    gen_gruop.plot.bar(color=['red','green','blue','orange'],
                        width=0.3)
    
    # 제목 정의
    plt.title("커피샵 별 선호도", fontsize=48)
    plt.xlabel("커피샵", fontsize=30)
    plt.ylabel("선호도 건수", fontsize=30)
    
    # x축, y축에 들어가는 값들에 대한 폰트 사이즈, 각도
